    ...
    <tr>
      ...
      <td align="middle">
       ...
       $flag = 0;
        ...
        $stmt = $pdo->query ('SELECT name_product
                              FROM product');
        while ($row = $stmt->fetch()) {
         if($row['name_product'] == $prod) { $flag = 1;}
                                      }

                      

       if(!$flag){
        $sql = "INSERT INTO product (name_product, meas_product, strg_product) VALUES (?,?,?)";
        $stmt= $pdo->prepare($sql);
        if($stmt->execute([$prod, $m, $s])){ 
          echo '<b><font color="#454545" size ="3">'.'Продукт добавлен!'.'</b></font><br><br>';
          ...
          echo '<b><font color="#454545" size ="3">'.'Такой продукт уже есть!'.'</font></b><br><br>';
