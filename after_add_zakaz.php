  ...
  <table width="100%" cellspacing="0" cellpadding="5">
    <tr>
      ...
      <?php
       ...
       $stmt = $pdo->query ('SELECT Cont,St,En FROM dogovora');
       while( $row = $stmt->fetch() ) {
        if($row['Cont'] == $con) {
         if($date >= $row['St'] and $date <= $row['En']){ $flag = 1;} 
                                 } 
                                      };   
       ...
       $row = $stmt->fetch();
        ...
        if($flag) {
        $sql = "INSERT INTO ord (id_cont, count, date,number_ord) VALUES (?,?,?,?)";
        $stmt= $pdo->prepare($sql);
        if($stmt->execute([$con, $count, $date,$num])){ 
          echo '<b><font color="#454545" size ="3">'.'Заказ добавлен!'.'</font><br>';
          echo'<img src="/WEB-APP/img/ok.png"width="370"height="280">'.'<br>';
          echo'<a href="/WEB-APP/script/zakaz/add_zakaz.php" class="button8" >Сделать ещё заказ</a><p>';
          echo'<a href="/WEB-APP/script/zakaz/postavki_act.php" class="button8" >Актуальные заказы</a>';
                                                                  } } else {

           echo '<b><font color="#454545" size ="3">'.'Дата не попадает в срок действия договора!'.'</font><br>';
