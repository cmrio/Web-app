...
<br>
  ...
  <table width="100%" cellspacing="0" cellpadding="5">
    <tr>
      <td align="left" width="15%" height="670" background="/WEB-APP/img/111.png">
       <a href="/WEB-APP/script/poisk/search_prod_dis.php" class="button8">Вернуться</a>
     </td>
       ...
       $stmt = $pdo->query ('SELECT Prod,D,Kol,Mea,Summa
                             FROM dis
                             ORDER BY D DESC');
       $ds = $_POST['ds'];
       $de = $_POST['de'];
       ...
       if( $de >= $ds) {
        ...
        $prod[$i][0] = $row['Prod'];
        $prod[$i][1] = $row['D']; 
        $prod[$i][2] = $row['Kol'].' '.$row['Mea'];
        $prod[$i][3] = $row['Summa'].' грн';  
        $i = $i + 1;       }
                                   } 
              
       if ($i != 0) {
     echo '<td align="middle" valign="top" width="60%">
           <table class="scroll" align="center" border="1">
            ...
            for($i = 0; $i < count($prod); $i++){
             echo '<tr>';
             echo '<td align="center"><font color="#454545" size="3"><b>'.$prod[$i][0].'</b></font></td>'.'<td align="center">'.$prod[$i][1].'</td>'.'<td align="center">'.$prod[$i][2].'</td>'.'<td align="center">'.$prod[$i][3].'</td>';
             echo '</tr>'; }
          
            echo '</tbody>
          </table>'; } else {
                  echo '<td align="center" bgcolor="#f8f6f1" width="60%">
