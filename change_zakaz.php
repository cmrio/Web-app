  ...
  <table width="100%" cellspacing="0" cellpadding="5">
    <tr>
      ...
      <?php
       ...
       ?>
            <font color="red" size ="4">Внимание: изменить можно лишь те заказы, которые ещё не состоялись!</font>
            <form action="/WEB-APP/script/zakaz/change_zakaz2.php" METHOD="post">
            <p><b><font color="#454545" size ="5">Выберите заказ</font></b><br>
            <p><select name="id_ord" size = "1" required>
             <?php
             $i = 0;
             $stmt = $pdo->query ('SELECT id_z,Prod,Post,CZ,Zakaz,Summa,Mea,D
                                   FROM zakazu
                                   WHERE D > CURRENT_DATE
                                   order by D ASC');
               while ($row = $stmt->fetch()) {
               $prod[$i] = 'Заказ: '.$row['Prod'].' '.$row['CZ'].' '.$row['Mea'].', у '.$row['Post'].', на сумму '.$row['Summa'].' грн, номер заказа: '.$row['Zakaz'];
               $idprod[$i] = $row['id_z'];   
               $i = $i + 1; }
        ...
        <INPUT TYPE="Submit" VALUE= "Изменить данные договора"> <br>
        </form>
