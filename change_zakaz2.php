  ...
  <table width="100%" cellspacing="0" cellpadding="5">
    <tr>
      ...
      <?php
       ...
       $stmt = $pdo->query ('SELECT id_ord,count,date,number_ord, dateend
                             FROM ORD INNER JOIN contract ON id_cont = id_con;');
          while ($row = $stmt->fetch()) {
          if($row['id_ord'] == $_POST['id_ord']) { $c = $row['count']; 
                                                   $d = $row['date'];
                                                   $n = $row['number_ord'];
                                                   $de = $row['dateend'];}}
            
     
              $stmt = $pdo->query ('SELECT CURRENT_DATE + INTERVAL 1 DAY;');
              $row = $stmt->fetch();
              $md = $row['CURRENT_DATE + INTERVAL 1 DAY'];
              
 
       ?>

       <form action="/WEB-APP/script/zakaz/change_zakaz3.php" METHOD="post">
            <b><font color="#454545" size ="5">Введите количество </font></b>
              <?php
               echo '<INPUT TYPE="number" name = "count" min = "1" required VALUE='.$c.'>';
              ?><p><br>
           
            <b><font color="#454545" size ="5">Введите дату </font></b>
              <?php
               echo '<INPUT TYPE="date" name = "date" min="'.$md.'" max = "'.$de.'" required VALUE='.$d.'>';
              ?>
            </select><p><br>

           <b><font color="#454545" size ="5">Введите номер заказа</font></b>
              <?php
              echo '<INPUT TYPE="number" name = "num" min = "1" required VALUE='.$n.'>';
              ?><p><br>

          <input type="hidden" name="ord" value="<?php echo $_POST['id_ord'] ?>"><br><br>

        <INPUT TYPE="Submit" VALUE= "Изменить данные заказа"  > <br>
        </form>
