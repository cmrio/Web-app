    ...
    <tr>
      ...
      <td align="middle">
       ...
       $flag = 0;              
        ...
        if($name == $row['name_product'] and $meas == $row['meas_product'] and $strg == $row['strg_product']) { $flag = 1;};
                                     } 
        ...
        if(!$flag){       
        $sql = "UPDATE product SET name_product = ?,meas_product = ?,strg_product = ? WHERE id_product = ?";
        $stmt= $pdo->prepare($sql);
        if($stmt->execute([$name, $meas, $strg, $_POST['id_prod']])){ 
          echo '<b><font color="#454545" size ="3">'.'Продукт изменен!'.'</font></b><br>';
          echo'<img src="/WEB-APP/img/ok.png"width="370"height="280">'.'<br>';
          echo'<a href="/WEB-APP/script/product/prod.php" class="button8" >Просмотр продуктов</a>';}
                         } else {
          echo '<b><font color="#454545" size ="3">'.'Такой продукт уже есть!'.'</font></b><br>';
