...
<head>
  ...
  <title>Не актуальные заказы</title>
  ...
  <table width="100%" cellspacing="0" cellpadding="5">
    <tr>
     <td align="left" width="20%">
        <a href="/WEB-APP/script/zakaz/postavki_pass.php" class="button8">Вернуться</a>
      </td>
      <?php
       $i = 0;
       $host = 'localhost';
       ...
       $stmt = $pdo->query ('SELECT Zakaz,Prod,Post,CZ,D,CC,Summa,Mea
                             FROM zakazu
                             WHERE (id_z not in (select id_ord_curr
                                              from currently))
                             ORDER BY D DESC ');
       $name = $_POST['name'];
      ...
      echo ' <td align="center">
            <table class = "scroll"  align="middle" border="1 ">
         <thead>
           <tr>
            <th>Номер заказа</td>
            <th>Продукт</td>
          ...
          <tbody>';
            ...
            for($i = 0; $i < count($prod); $i++){
             echo '<tr>';
             echo '<td align="center">'.$prod[$i][0].'</td>'.'<td align="center"><font color="#454545" size ="3"><b>'.$prod[$i][1].'</b></font></td>'.'<td align="center">'.$prod[$i][2].'</td>'.'<td align="center">'.$prod[$i][3].'</td>'.'<td align="center">'.$prod[$i][4].'</td>'.'<td align="center">'.$prod[$i][5].'</td>'.'<td align="center">'.$prod[$i][6].'</td>';
             echo '</tr>'; }  
          echo'</body>
         </table>'; }  else {
                  echo '<td align="middle" bgcolor = "#f8f6f1" width="30%" >
