...
<body>
  ...
  <table width="100%" cellspacing="0" cellpadding="5">
    <tr>
     <td align="left" width="8%" height="670" background="/WEB-APP/img/111.png">
       <a href="/WEB-APP/script/poisk/cont_date.php" class="button8">Вернуться</a>
     </td>
      ...
      <?php
       ...
       $stmt = $pdo->query ('Call auto_end_cont');
      
       $stmt = $pdo->query ('SELECT Prod, Prov, Pric, St, En, Kol, Sta
                             FROM dogovora ');
        $flagD = 1;

        if($_POST['dates'] > $_POST['datee']) { $flagD = 0;};
        
        if($flagD) {

         
        while ($row = $stmt->fetch()) {
        if( $row['St'] >= $_POST['dates'] and $row['En'] <= $_POST['datee']){
        $prod[$i][0] = $row['Prod']; 
        $prod[$i][1] = $row['Prov']; 
        $prod[$i][2] = $row['Pric'];
        $prod[$i][3] = $row['St'];
        $prod[$i][4] = $row['En'];
        $prod[$i][5] = $row['Kol'];
        $prod[$i][6] = $row['Sta'] == 1 ? "Активный" : "Не активный";
        $i = $i + 1;    }
                                      } 
              
      if($i != 0) {

    echo '<td align="middle" valign="top" width="72%">
       <table class = "scroll" border="1" >
         <thead>
           <tr>
            <th>Продукт</td>
            <th>Поставщик</td>
            <th>Цена, грн</td>
            <th>Начало договора</td>
            <th>Конец договора</td>
            <th>Количество поставок</td>
            <th>Статус</td>
          </tr>
          <thead>
          <tbody>';
            for($i = 0; $i < count($prod); $i++){
             echo '<tr>';
             echo '<td align="center"><font color="#454545" size ="3">'.$prod[$i][0].'</font></td>'.'<td align="center"><font size ="1"><b>'.$prod[$i][1].'</b></font></td>'.'<td align="center">'.$prod[$i][2].'</td>'.'<td align="center">'.$prod[$i][3].'</td>'.'<td align="center">'.$prod[$i][4].'</td>'.'<td align="center">'.$prod[$i][5].'</td>'.'<td align="center">'.$prod[$i][6].'</td>';
             echo '</tr>'; }
          echo'</body>
       </table>'; } else {
                  echo '<td align="center" bgcolor="#f8f6f1" width="72%">
                        <font color="#454545" size ="6"><b>Ничего не найдено</b></font>';}

              } else {
                  echo '<td align="center" bgcolor="#f8f6f1" width="72%">
                        <font color="#454545" size ="6"><b>Даты введены не корректно</b></font>';} ?>
    </td>
         <td align="left" background="/WEB-APP/img/111.png" width="20%" height="670" valign="center">
