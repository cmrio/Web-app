  ...
  <table width="100%" cellspacing="0" cellpadding="5">
    <tr>
      <td align="left" width="8%" >
       <a href="/WEB-APP/script/poisk/search_ord.php" class="button8">Вернуться</a>
     </td>
       ...
       $stmt = $pdo->query ('SELECT Zakaz,Prod,Post,CZ,D,CC,Summa,Mea,Sta
                             FROM search_zakaz');

       $pr = $_POST['prov'];

        while ($row = $stmt->fetch()) {
        if( mb_strtoupper(substr($row['Post'],0,strlen($pr))) == mb_strtoupper($pr) ){
        if($row['Sta']==1) {
        $prod[$i][0] = $row['Zakaz'].'*';
        $prod[$i][5] = $row['CC'].' '.$row['Mea'];
                                                  } else { $prod[$i][0] = $row['Zakaz'];
                                                           $prod[$i][5] = '0'.' '.$row['Mea']; };$prod[$i][1] = $row['Prod']; 
        $prod[$i][2] = $row['Post']; 
        $prod[$i][3] = $row['CZ'].' '.$row['Mea'];
        $prod[$i][4] = $row['D'];
        $prod[$i][6] = $row['Summa']; 
        $i = $i + 1;     }  
                                   } 
       ...
       if ($i != 0) {
     echo '<td align="middle" valign="top" width="70%">
           <table class="scroll" align="center" border="1">
            <thead>
              <tr>
                <th>Номер заказа<font size="2"> * - актуальный</font></td>
                <th>Продукт</td>
                <th>Поставщик</td>
                <th>Количество при заказе</td>
                <th>Дата заказа</td>
                <th>Количество на складе</td>
                <th>Сумма заказа, грн</td>
              </tr>
              <thead>
                <tbody>';
            ...
            for($i = 0; $i < count($prod); $i++){
             echo '<tr>';
             echo '<td align="center"><b>'.$prod[$i][0].'</b></td>'.'<td align="center"><font color="#454545" size ="3"><b>'.$prod[$i][1].'</b></font></td>'.'<td align="center">'.$prod[$i][2].'</td>'.'<td align="center">'.$prod[$i][3].'</td>'.'<td align="center">'.$prod[$i][4].'</td>'.'<td align="center">'.$prod[$i][5].'</td>'.'<td align="center">'.$prod[$i][6].'</td>';
             echo '</tr>'; }
          
            echo '</tbody>
          </table>'; } else {
                  echo '<td align="center" bgcolor="#f8f6f1" width="70%">
