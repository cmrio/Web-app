  ...
  <table width="100%" cellspacing="0" cellpadding="5">
    ...
    <td align="left" valign="middle">
      <?php
       ...
       <form action="after_add_zakaz.php" METHOD="post">
         <b><font color="#454545" size ="5">Выберите договор</font></b><br>
         <select name="con" size = "1" required>
         <?php
          $i = 0;
          $stmt = $pdo->query ('SELECT name_prov, name_product, price, id_con,meas_product
                                FROM provider INNER JOIN (product INNER JOIN contract ON 
                                product.id_product = contract.prod) ON provider.id_prov = contract.prov
                                WHERE contract.flag = 1');
            while ($row = $stmt->fetch()) {
            $prod[$i] = 'Заказать '.$row['name_product'].' у '.$row['name_prov'].' по цене: '.$row['price'].' грн за 1 '.$row['meas_product'];
            $idprod[$i] = $row['id_con'];  
            $i = $i + 1; }
          ...
          <INPUT TYPE="number" name = "count" min = "1" required > <p><br>
          
          <b><font color="#454545" size ="5">Введите дату</font></b>
          <INPUT TYPE="date" name = "date" min ="<?php echo $row['current_date'] ?>" required > <p><br>        
         
          <b><font color="#454545" size ="5">Введите номер заказа</font></b>
          <INPUT TYPE="number" name = "num" min="1" required> <br><p><br>

        <INPUT TYPE="Submit" VALUE= "Добавить заказ" > <br>
        </form>
     </td>
             <td background="/WEB-APP/img/111.png" width="300" height="670">
