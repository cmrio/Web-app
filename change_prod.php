    ...
    <td align="left" valign="middle">
      <?php
       ...
       ?>
          ...
          <form action="/WEB-APP/script/product/after_change_prod.php" METHOD="post">
             <b><font color="#454545" size ="5">Выберите продукт</font></b><br>
             <select name="id_prod" size="1" required>
                    <?php
             $i = 0;
             $stmt = $pdo->query ('SELECT id_product,name_product, meas_product, strg_product 
                                   FROM product
                                   ORDER BY name_product ASC');
               while ($row = $stmt->fetch()) {
               $prod[$i] = $row['name_product'].', размерность: '.$row['meas_product'].'. Текущий срок хранения: '.$row['strg_product'];
               $idprod[$i] = $row['id_product'];  
               $i = $i + 1; }
              ...
              ?>
                ...
                <b><font color="#454545" size ="5">Введите новое название</font></b>
                <INPUT TYPE="text" name="name" required size="26"> <p><br>
                    
                <b><font color="#454545" size ="5">Выберите новую размерность</font></b>
                        <select name="meas" size="1" required>
                 ...
                 <b><font color="#454545" size ="5">Введите новый срок хранения</font></b>
                        <INPUT TYPE="number" name="strg" min="1" required>
                        <br>
                        <br>
                        <br>
